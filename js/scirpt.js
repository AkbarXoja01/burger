...
const hi = document.querySelector(".header__img");
const ci = document.querySelectorAll(".card__img");

ci.forEach((img) => {
  img.addEventListener("click", () => {
    const att = img.getAttribute("src");
    hi.setAttribute("src", att);
  });
});

ci.forEach((img) => {
  img.addEventListener("dblclick", () => {
    const newatt = img.getAttribute("src");
    if (newatt.style.width === "180px") {
      newatt.style.width = "60px";
      newatt.style.height = "auto";
    } else {
      newatt.style.width = "180px";
      newatt.style.height = "auto";
    }
  });
});
